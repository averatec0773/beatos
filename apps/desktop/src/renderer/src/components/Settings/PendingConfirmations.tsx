...
export function PendingConfirmations({ tokens, onApprove, onReject }: Props): React.JSX.Element | null {
  if (tokens.length === 0) return null;
  return (
    <div className="mt-3">
      <div className="mb-2 text-sm font-medium text-danger">
        ⚠ Pending confirmations ({tokens.length})
      </div>
      <ul className="space-y-2">
        {tokens.map((t) => (
          <li key={t.token} className="rounded border border-border-subtle bg-bg-elevated p-3 text-xs">
            ...
            <div className="mt-2 flex gap-2">
              <button
                ...
                className="rounded border border-success px-2 py-0.5 text-success hover:bg-success/10"
              >
                ✓ Approve
              </button>
              <button
                ...
                className="rounded border border-danger px-2 py-0.5 text-danger hover:bg-danger/10"
              >
                ✗ Reject
              </button>
            </div>
