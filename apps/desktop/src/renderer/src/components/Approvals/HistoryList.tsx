    ...
    <section>
      ...
      <ul className="space-y-1">
        {tokens.map((t) => {
          ...
          return (
            <li key={t.token} className="flex items-center gap-3 px-3 py-1.5 text-xs text-text-secondary">
              <span className={`text-base ${statusClass(t.status)}`}>{statusGlyph(t.status)}</span>
              <span className="font-mono">{t.tool_name} {formatPayload(t.tool_name, t.payload)}</span>
