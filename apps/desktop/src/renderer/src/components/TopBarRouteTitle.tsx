...
export function TopBarRouteTitle(): React.JSX.Element {
  ...
  let title = "BeatOS";
  if (location.pathname === "/") title = "All Beats";
  else if (location.pathname === "/settings") title = "Settings";
  else if (matchPath("/tracks/new", location.pathname)) title = "New Track";
  else if (matchPath("/tracks/:id/edit", location.pathname)) title = "Editor";
  else {
