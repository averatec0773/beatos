...
describe("SettingsPanel", () => {
  ...
  describe("Producers section", () => {
    it("renders each producer as a chip", async () => {
      ...
      render(<MemoryRouter><SettingsPanel /></MemoryRouter>);
      await waitFor(() =>
        expect(screen.getAllByTestId("producer-chip")).toHaveLength(3),
      );
    ...
    it("shows empty-state copy when there are no producers yet", async () => {
      ...
      render(<MemoryRouter><SettingsPanel /></MemoryRouter>);
      await waitFor(() =>
        expect(screen.getByText(/No producers yet/i)).toBeInTheDocument(),
      );
    });

    it("Remove on a used producer calls rewrite + refreshes the list", async () => {
      const distinctSpy = vi.spyOn(distinct, "values")
