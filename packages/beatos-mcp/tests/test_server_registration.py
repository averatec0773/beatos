...
async def test_list_tools_includes_create_list_and_await_approval():
    ...
    assert "attach_asset" in names
    assert "draft_descriptions" in names
    # 5 read tools + create_list + await_approval + 3 lifecycle tools + 5 list-curation tools + 2 metadata tools + 2 ingest tools + 1 AI-content tool
    assert len(tools) == 20
