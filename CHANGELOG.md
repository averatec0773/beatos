...
- **Agent-attached audio is usable again.** Attaching an audio file to a track over MCP (`attach_assets` with `role: "audio"`) stored a non-canonical role, so the file was invisible to playback, analysis, and download — the two-approval folder-import flow silently produced unplayable tracks. The role now resolves to the correct audio type by file extension, and unsupported audio extensions (`.flac`/`.aiff`, which have no representable role) are rejected up front.
