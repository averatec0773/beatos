...
- **Web frontend (Phase 0, experimental).** The renderer now also builds as a browser SPA served by the local sidecar — so BeatOS runs cross-platform (incl. Windows) without an Electron build, sharing the same backend and a near-identical UI. One renderer codebase, two build targets: Electron-only capabilities route through a `platform` seam, and WAV sanitization moved server-side so browsers decode DAW WAVs. This first slice covers the library / search / playback / metadata-edit flow; browser file import/export comes next.
