...
- **FLAC support — audio format decoupled from role.** The audio file format (now **WAV / MP3 / FLAC**) is a separate attribute of an asset rather than being baked into its role. A track can hold the same slot in multiple formats, the editor gains **FLAC (tagged/untagged)** slots, and agents attaching audio over MCP accept `.flac` too. Adding another format later is a one-line change, not a new role across every layer.
