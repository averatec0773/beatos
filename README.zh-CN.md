...
[![MCP](https://img.shields.io/badge/MCP-Claude%20%E2%80%A2%20Codex%20%E2%80%A2%20any%20client-7c5cff?style=flat-square)](#把-ai-用起来的两种方式)
...
<div align="center">
  <br/>
  <img src="screenshots/beatos-core-product-demo.gif" alt="BeatOS 演示：库内搜索、播放、loopkit 导出、Publish Center 与 AI Agent" width="1100" />
  <br/><br/>
</div>

## 为什么是 BeatOS

我自己就是个 beatmaker。和大多数人一样，我的库曾经躺在一张表格里，而同样的元数据要一条条手动敲进每个我卖货的平台。BeatOS 就是我为自己造的替代品——用**一份规范的总目录**，把别处不会同时具备的三件事合在一起：

- 🎯 **为卖 beat 而生**——带多币种定价的 license tier、producer 署名、tagged/untagged/loop/stems、BPM 与 Key。不是把一个通用媒体管理器硬掰成这个样子。
- 🤖 **AI 原生**——用大白话和你的目录对话，从封面起草 genre/mood/tag，或接入 MCP 让 Claude、Codex 帮你干活。每一次写入都等你批准。
...
- **瞬间找到任何一条 beat。** 在整个目录里搜索并叠加筛选——BPM 区间、Key、genre、mood、producer、有无音频——最近搜索一键可达。播放队列跟随你正在看的内容。

- **Loopkit、beattape 与 beat pack。** 把若干 beat 整理成一个列表，然后**打包寄出**——一份 loopkit、一份 beattape、一份给歌手的 beat pack。逐条、逐文件挑选要放进去的内容（批量勾选所有 WAV / 所有 MP3），再导出为 **ZIP** 或普通文件夹，每条 beat 一个子文件夹。

- **分析与 AI 打标签。** 按需进行 **BPM + Key 检测**，带逐字段置信度评分（Essentia 引擎，或宽松许可的 librosa 后备）——分析单条，或批量处理整个库，自动补全空字段。可选的 **AI 标签建议**（自带 key）从封面 + 标题起草 genre/mood/tag，供你逐字段审阅。

- **和你的目录对话。** 侧边栏内置 **AI Agent** 聊天——用大白话提问（*"找出我 2024 年没打标签的 beat"*、*"把这些标成 chill"*、*"加一个 WAV 档位"*），模型会真的去读取并编辑你的目录。支持 **Claude、ChatGPT 或 DeepSeek**，用你自己的 key、只存在你的设备上。编辑按你的权限设置生效并记录在案；破坏性操作会先让你确认；只读模式会拒绝写入。对话保存在本地，下次启动可恢复。

- **从你自己的 AI 里驱动它。** 一流的 **MCP 服务器**把库暴露给 Claude Code、Claude Desktop、Codex 以及任何 MCP 客户端——免费 **23 个工具，Pro 共 28 个**——让终端里的 agent 替你整理目录、打标签、准备每个平台的元数据。

- **辅助发布（Pro）。** 无需离开 BeatOS 即可发布一条 beat：引擎驱动真实浏览器，并在平台的验证环节暂停，**Publish Center** 实时显示每个平台的会话健康度。当前可用目标：**抖音（Douyin）宣传视频与网易 激灵（BeatSoul）**，**BeatStars** 在[路线图](ROADMAP.md)上。仅 Pro 构建——免费构建会将其置灰。
...
两者都是 **local-first 且离线**——浏览器应用只与 `127.0.0.1` 通信。（远程/局域网访问与移动端布局在计划中。）

## 把 AI 用起来的两种方式

> **BeatOS 为 AI agent 时代而建。** 整理目录、打标签以及（即将到来的）发布，都旨在无需手工劳动即可规模化。

**应用内**——上面的 **AI Agent** 聊天只需要你自己的 API key（Claude / ChatGPT / DeepSeek）。在设置里打开 AI Assist、粘贴 key，就能开始提问。读取直接执行；编辑按你的权限模式生效并记录在案。

**你自己的 AI 客户端**——通过 **MCP** 接入。已验证客户端：Claude Code · Claude Desktop · Codex CLI/App · 任何讲 stdio JSON-RPC 的客户端。

**无论哪种方式，主动权都在你手里。** 你的 MCP 客户端为每一次工具调用把关（逐次允许 / 拒绝），且应用的每一次写入都记录在 **Agent Actions** 看板里——改了什么、何时、结果如何。想要硬性停止？把 agent 切到**只读**，写入会被直接拒绝。批量工具（`create_tracks` ≤100、`attach_assets` ≤500）把一次 50 条的导入变成一个动作，而不是一百个。
...
- **无服务器。** sidecar 绑定 `127.0.0.1` 并 same-origin 提供浏览器前端。没有东西离开本机——包括你与 AI 的对话。
- **无账号。** 单用户。无登录、无同步、无云。
- **无遥测。** 应用自身零外发请求；你的 API key 只留在你的设备上。
