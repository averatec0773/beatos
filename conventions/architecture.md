...
| `app_setting` table | `packages/beatos-core/beatos_core/migrations/014_app_setting.sql` | Generic catalog-level key/value JSON store. First consumer: `default_license_tiers` — the templates auto-applied to every renderer-created track (`stores/tracks.ts::create` + `lib/create-track-from-file.ts`). MCP `create_tracks` intentionally does NOT pull these defaults (agents typically want full control over the tier set they're importing). Second consumer (v0.0.36): `vocab_locale` (`both`/`zh`/`en`, default `both`) — the renderer-only genre/mood display language. Hydrated at boot into `stores/vocab-locale.ts`; every display site routes labels through the single helper `data/vocab-label.ts::formatVocabLabel(value, kind, locale)`. Stored genre/mood values stay English-canonical and NetEase export is unaffected. Third consumer (v0.0.40, refined v0.0.41): `upload_templates` — `{album_name, beat_name, beat_description, album_description, prod_separator}` strings (Settings → 上传模板, `stores/upload-templates.ts`). Resolved by `beatos_core/export/service.py::_resolve_templates` (per-field merge over `export/templates.py::DEFAULT_TEMPLATES`) and rendered by the pure `export/templates.py::render_template` (tokens `{title}{genre}{year}{publish date}{prod}{bpm}{key}` — regex allows spaces in `{...}`; year + publish_date injected by the service so the renderer stays clock-free), threaded into `netease.render` for the `album_name`/`album_description`/`title`/`description` fields. Fourth consumer (v0.0.43): `primary_producer` — a single producer name marked "you" (★) in Settings → Producers (`lib/known-producers.ts::loadPrimaryProducer`/`savePrimaryProducer`). `{prod}` resolves from `track.producer` joined by `prod_separator` (default ` x `) with the primary producer sorted **first**, falling back to the primary name alone when the track has no producer (`service.py::_resolve_prod(track, *, primary, separator)`, primary read from `primary_producer` by `export_metadata`). The old `upload_templates.prod` fallback field was removed in v0.0.43 (superseded by `primary_producer`). |
