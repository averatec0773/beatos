...
| Trash UI | `routes/TrashPanel.tsx` + `stores/trash.ts` + `SidebarPanel.tsx` TRASH section | Sidebar section with live count; row-level Restore + Delete forever (confirm). List membership preserved across delete/restore. |
...
| Sidebar drag-reorder | `routes/SidebarPanel.tsx` `SortableSourceRow` / `SortableListRow` via `@dnd-kit/sortable` | System list "All Beats" (`kind='system'`) excluded. Track→list droppable id `list-drop:N` to avoid colliding with sortable id `list:N`. |
