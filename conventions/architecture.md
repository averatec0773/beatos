...
| `license_tier.share` | `packages/beatos-core/beatos_core/migrations/017_license_tier_share.sql` | v0.0.42 nullable `REAL` revenue-share percentage (网易云 编曲分润比例), 0–100; NULL = unset (append-only; existing rows NULL). Validated by `licenses/service.py::_normalize_share`; `_SELECT_COLS` appends it LAST so existing `_row_to_tier` indices are unchanged. `netease.render` emits a structured `price_tiers` ExportField (deliverables → `mp3`/`wav`/`stem` row key, CNY price, share) that the extension's multi-row `license-modal` driver fills into the 授权设置 rental matrix (exact-title-first row match; 永久独家 + unlimited row stay manual). |
| `app_setting` table | `packages/beatos-core/beatos_core/migrations/014_app_setting.sql` | Generic catalog-level key/value JSON store. First consumer: `default_license_tiers` — the templates auto-applied to every renderer-created track (`stores/tracks.ts::create` + `lib/create-track-from-file.ts`). MCP `create_tracks` intentionally does NOT pull these defaults (agents typically want full control over the tier set they're importing). Second consumer (v0.0.36): `vocab_locale` (`both`/`zh`/`en`, default `both`) — the renderer-only genre/mood display language. Hydrated at boot into `stores/vocab-locale.ts`; every display site routes labels through the single helper `data/vocab-label.ts::formatVocabLabel(value, kind, locale)`. Stored genre/mood values stay English-canonical and NetEase export is unaffected. Third consumer (v0.0.40, refined v0.0.41): `upload_templates` — `{album_name, beat_name, beat_description, album_description, prod_separator, free_prefix}` strings (Settings → 上传模板, `stores/upload-templates.ts`). Resolved by `beatos_core/export/service.py::_resolve_templates` (per-field merge over `export/templates.py::DEFAULT_TEMPLATES`) and rendered by the pure `export/templates.py::render_template` (tokens `{title}{genre}{year}{publish date}{prod}{bpm}{key}{free}` — regex allows spaces in `{...}`; year + publish_date injected by the service so the renderer stays clock-free), threaded into `netease.render` for the `album_name`/`album_description`/`title`/`description` fields. Fourth consumer (v0.0.43): `primary_producer` — a single producer name marked "you" (★) in Settings → Producers (`lib/known-producers.ts::loadPrimaryProducer`/`savePrimaryProducer`). `{prod}` resolves from `track.producer` joined by `prod_separator` (default ` x `) with the primary producer sorted **first**, falling back to the primary name alone when the track has no producer (`service.py::_resolve_prod(track, *, primary, separator)`, primary read from `primary_producer` by `export_metadata`). The old `upload_templates.prod` fallback field was removed in v0.0.43 (superseded by `primary_producer`). Fifth consumer (v0.0.44): `default_is_free` (boolean) — the create-time default for the per-track `is_free` flag (track column, migration 018), applied post-create by `lib/default-free.ts::applyDefaultIsFree` (mirrors `applyDefaultLicenseTiers`). A free track renders the `{free}` token as `free_prefix` (default `[FREE] `) and, on NetEase export, carries an `is_free` ExportField that makes the extension check the **免费使用** option in the 授权设置 drawer (multi-select: 免费使用 / 租赁授权 / 永久独家) alongside the paid rental tiers. |
...
**Phase 2-B (v0.0.38) — custom-widget click-interaction fill.** The extension gained typed async "widget drivers" (`apps/extension/src/interaction-fill.ts`) for the click-to-open Ant widgets Phase 1 left manual: `antv3-select` (曲风), `key-triple` (音名/调号/调式 via `key-decompose.ts` + per-part labelMaps `♯/♭/无`, `Major/Minor`), `tag-modal` (说明标签 — clicks NetEase's fixed scene-tag buttons matching mood+tags), `license-modal` (价格 — `price-parse.ts` tiers, best-effort). Triggers resolve **by field label** (`findControlByLabel`) since Ant selects lack stable ids; options match by text (`prefix` for bilingual 曲风 options). `dom-wait.ts::waitFor` polls for the portal/modal after each click; drivers run sequentially (one popup at a time), never throw (orchestrator try/catch → `missed`), and never submit. `content.ts` runs sync `fillForm` (native) then `await fillInteractions`, merging both reports. Recipes (selectors + labelMaps + tierMap) are data in `upload-form.json`. **Zero sidecar change** — `/api/inject/form-map` just serves the richer recipe. 曲风/KEY reliable; 说明标签/价格 best-effort. Later additions: `license-modal` also checks 免费使用 when the export's `is_free="1"` (v0.0.44); `album-create` (extension 0.2.7) opens the custom 专辑 selector (`#common-album-selector`, via `focus+mousedown+mouseup+click` — a bare click is ignored), clicks 创建新专辑, and fills 专辑名称 + 专辑描述 (always new album; desc skipped if <10 字; cover + 提交 stay manual — the album is created by the human's beat submit, not a separate 确定).
