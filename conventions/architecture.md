...
| **Adapter / Inject** | Per-platform upload integration. The earlier browser-extension inject is retired; platform publishing is now a Pro feature in the private `packages/pro/` submodule — details live in the private repo. |
...
**Platform publishing** is a Pro feature in the private `packages/pro/` submodule; details live in the private repo.
...
- **Legacy inject read-port (48923) serves `read_router` only.** The retired browser extension used this fixed read-only sidecar port; it must never expose write endpoints — it's an unauthenticated localhost port reachable by any local process. The extension is gone and platform publishing is now a Pro feature (private `packages/pro/` submodule). Don't build new features on this legacy port.
